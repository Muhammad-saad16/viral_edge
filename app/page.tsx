...
function WorkCardWrapper({ isPak, href, style, children }: {
  isPak: boolean; href: string; style?: React.CSSProperties; children: React.ReactNode;
}) {
  const cls = isPak
    ? "relative flex flex-col h-full rounded-3xl overflow-hidden cursor-default"
    : "group relative flex flex-col h-full rounded-3xl overflow-hidden";
  if (isPak) return <div className={cls} style={style}>{children}</div>;
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-10">
          ...
          >
            {activeWorks.map((w, i) => (
              ...
              >
                <TiltCard>
                  <WorkCardWrapper isPak={region === "PAK"} href="/portfolio"
                    ...
                    <div className="relative h-44 overflow-hidden flex-shrink-0"
                      ...
                      {w.logo && (
                        <div className="absolute inset-0 z-[5]">
                          <WorkLogoImage src={w.logo} alt={w.title} />
                        </div>
                      )}
